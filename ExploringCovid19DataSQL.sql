...
SELECT continent, MAX(CAST(total_deaths AS INT)) AS Continent_Death_Count
FROM PortfolioProject..CovidDeath
WHERE continent IS NOT NULL
GROUP BY continent
...
--Progress of Vaccinations at each country
...
--Some countries with incorrect new_vaccinations which leads to un accurate vaccinations precent from population.
